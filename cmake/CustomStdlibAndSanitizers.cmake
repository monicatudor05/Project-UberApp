...
function(set_custom_stdlib_and_sanitizers target add_apple_asan)
    ...
    if(APPLE)
        ...
        if("${CMAKE_CXX_COMPILER_ID}" MATCHES "Clang" AND add_apple_asan MATCHES true)
            if(USE_ASAN)
                target_compile_options(${target} PRIVATE "$<${debug_mode}:-fsanitize=address>")
                target_link_options(${target} PRIVATE "$<${debug_mode}:-fsanitize=address>")
            endif()
        endif()
    elseif(UNIX)
        if(USE_ASAN)
        ...
        elseif(USE_MSAN)
            ...
            target_compile_options(${target} PRIVATE "$<${debug_mode}:-fsanitize=memory;-fsanitize-recover=memory;-fsanitize-memory-track-origins;-fsanitize-ignorelist=${MSAN_IGNORE_PATH}>")
            target_link_options(${target} PRIVATE "$<${debug_mode}:-fsanitize=memory;-fsanitize-recover=memory;-fsanitize-memory-track-origins>")

            # This tells MSan to keep running even if it hits uninitialized memory in uninstrumented libs
            set_property(TARGET ${target} PROPERTY PROPERTY TEST_ENV "MSAN_OPTIONS=halt_on_error=0")
        endif()
    endif()
endfunction()
